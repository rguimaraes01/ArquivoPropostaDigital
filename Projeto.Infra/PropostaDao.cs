...
namespace Projeto.Infra
{
    public class PropostaDao
    {
        public List<Proposta> GetPropostas(string usuario, string nomeArquivo)
        {
            ...
            var propostasDigitacao = digitacaoContext1.TB_PROPOSTA.Where(p => p.tp_motdve.Equals("9")).ToList();
            ...
            Parallel.Invoke(() =>
            {
                if (propostasDigitacao.Count() > 0)
                {
                    ...
                    Parallel.ForEach(propostasDigitacao, propostaDigitacao =>
                    {
                        ...
                        proposta.dadosCadastrais = new DadosCadastrais(erro)
                        {
                            ...
                            //conjuge = conjuge == null ? null : this.InstanciaConjuge(conjuge, profissaoConjuge, clienteTitular?.nr_cgccpfcli.ToString().PadLeft(11, '0')),
                            //filhos = filhos.Any() == false ? null : this.PercorreFilhos(filhos),
                        ...
                        proposta.coberturaTitular = new CoberturaTitular()
                        {
                            capitalSeguradoMorteAcidencial = clienteTitular.vl_isap?.ToString("N2", CultureInfo.CreateSpecificCulture("pt-BR")),
                            valorPremioAcidentesPessoais = clienteTitular.vl_pmo?.ToString("N2", CultureInfo.CreateSpecificCulture("pt-BR")),
                            capitalSeguradoAssistenciaEmergencial = clienteTitular.vl_isdoecgn?.ToString("N2", CultureInfo.CreateSpecificCulture("pt-BR")),
                            capitalSeguradoAssistenciaFuneral = clienteTitular.vl_isalodtr?.ToString("N2", CultureInfo.CreateSpecificCulture("pt-BR")),
                            valorPremioAssistenciaEmergencial = clienteTitular.vl_pmodoecgn?.ToString("N2", CultureInfo.CreateSpecificCulture("pt-BR")),
                            valorPremioAssistenciaFuneral = clienteTitular.vl_pmoalodtr?.ToString("N2", CultureInfo.CreateSpecificCulture("pt-BR"))
                        };
                        proposta.agregados = agregados.Any() == false ? null : GeraAgregados(agregados, dominioCampo, erro);
                        ...
                        proposta.calculoPremioTotal = new CalculoPremioTotal(erro)
                        {
                            ...
                            valorPremioRendaMensal = propostaDigitacao.vr_cobadc.Value.ToString("N2", CultureInfo.CreateSpecificCulture("pt-BR")),
                            valorPremioTotal = this.RetornaPremioTotal(clienteTitular, agregados, propostaDigitacao).ToString("N2", CultureInfo.CreateSpecificCulture("pt-BR")),
                            valorPremioAssistenciaFuneral = "0",
                            valorPremioFuneralMaster = "0"
                        };
                        ...
                        proposta.rendaMensal = this.InstanciaRendaMensal(propostaDigitacao, erro);


                        proposta.coberturaConjuge = conjuge == null ? new CoberturaConjuge() : new CoberturaConjuge()
                        {
                            plano = clienteTitular.ds_pla,
                            capitalSeguradoMorteAcidencial = conjuge.vl_isap.Value.ToString("N2", CultureInfo.CreateSpecificCulture("pt-BR")),
                            valorPremioAcidentesPessoais = conjuge.vl_pmo.Value.ToString("N2", CultureInfo.CreateSpecificCulture("pt-BR")),
                            capitalSeguradoAssistenciaEmergencial = conjuge.vl_isdoecgn.Value.ToString("N2", CultureInfo.CreateSpecificCulture("pt-BR")),
                            capitalSeguradoAssistenciaFuneral = conjuge.vl_isalodtr.Value.ToString("N2", CultureInfo.CreateSpecificCulture("pt-BR")),
                            valorPremioAssistenciaEmergencial = conjuge.vl_pmodoecgn.Value.ToString("N2", CultureInfo.CreateSpecificCulture("pt-BR")),
                            valorPremioAssistenciaFuneral = conjuge.vl_pmoalodtr.Value.ToString("N2", CultureInfo.CreateSpecificCulture("pt-BR")),
                        };
                        proposta.rendaMensalConjuge = conjuge == null ? null : this.InstanciaRendaMensalConjuge(conjuge.TB_PROPOSTA, erro);
                        ...
                        proposta.beneficiarios = beneficiariosCliente.Any() == false ? null : geraBeneficiarios(digitacaoContext2, propostaDigitacao.nr_ppt, erro); 
        ...
        private Conjuge InstanciaConjuge(TB_CLIENTE clienteTipoTres, AtivPessoa ativPessoa, string CPF)
        {
            ...
            conjuge = new Conjuge
            {
                nome = clienteTipoTres?.nm_nmecli,
        ...
        */
        ...
        private Filhos InstanciaFilho(TB_CLIENTE filhos)
        {
            Filhos filho = new Filhos
            {
                nome = filhos.nm_nmecli,
                dataNascimento = filhos.dt_nsccli?.ToShortDateString(),
                sexo = this.deParaSexo(filhos.tp_sexocli)
            };

            return filho;
        }

        private List<Filhos> PercorreFilhos(List<TB_CLIENTE> tbFilhos)
        {
            List<Filhos> listaFilhos = new List<Filhos>();

            foreach (TB_CLIENTE tbFilho in tbFilhos)
            {
                Filhos filho = this.InstanciaFilho(tbFilho);
        ...
        private List<Beneficiario> geraBeneficiarios(DigitacaoEntities digitacaoEntities, int nr_ppt , Erro erro)
        {
            ...
            foreach (var item in listaTB_Beneficiario)
            {
                ...
                beneficiario.Percentual = item.pr_ptcben.Value.ToString("N2", CultureInfo.CreateSpecificCulture("pt-BR"));
        ...
        private decimal RetornaPremioTotal(TB_CLIENTE acidentesPessoais, List<TB_CLIENTE> premioAgregados, TB_PROPOSTA rendaMensal)
        {
            var premioAcidentesPessoais = (decimal)acidentesPessoais.vl_pmo;
            var somaPremioAgregados = this.SomaValorAgregados(premioAgregados);
            var somaPremioRendaMensal = (decimal)rendaMensal.vr_cobadc;

            return Math.Round(premioAcidentesPessoais + somaPremioAgregados + somaPremioRendaMensal, 2);
        }
